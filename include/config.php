...
** Author by Aninda Saputri
** Bootstrap Template by Edmin - EGrappler
**/ $version = "1.2"; $build_year="2021"; 
...
/** Application Information **/
	...
	//database connection
	$connect	= mysqli_connect('localhost',$cfg['dbUser'],$cfg['dbPass']);
	mysqli_select_db($connect,$cfg['dbName']);
...
/**jangan mengubah data dibawah agar aplikasi tetap berjalan dengan baik **/
		...
		function copyright(){
			...
			if(date("Y") > $build_year){ 
				...
				echo "<script>
							alert('Tanggal anda tidak akurat, Aplikasi mungkin tidak berjalan dengan baik. Perbaiki setelan tanggal anda!')
							</script>";
			...
			echo '</b><br/>Arsip Surat By Aninda Saputri.';
		}
